        ...
        for keyword in keywords {
            ...
            if let range = lowerText.range(of: keyword, range: searchRange) {
                ...
                if let originalRange = rangeInOriginalText(text, lowerRange: range) {
                    ...
                    lastEndIndex = originalRange.upperBound
                    totalScore += 1.0
                }
            }
            // 尝试前缀匹配
            else if let prefixRange = findPrefixMatch(in: text, keyword: keyword, from: searchStart) {
                ranges.append(prefixRange)
                lastEndIndex = prefixRange.upperBound
                totalScore += 0.8
            }
            // 尝试近似匹配（编辑距离）
            else if let approximateRange = findApproximateMatch(
                in: text,
                keyword: keyword,
                from: searchStart,
                maxDistance: max(1, keyword.count / 3)
            ) {
                ranges.append(approximateRange)
                lastEndIndex = approximateRange.upperBound
                totalScore += 0.5
            } else {
                // 关键词不匹配
    ...
    static func findAllMatchRanges(in text: String, keyword: String) -> [Range<String.Index>] {
        ...
        for keyword in keywords {
            ...
            while searchStart < lowerText.endIndex {
                let searchRange = searchStart..<lowerText.endIndex

                guard let range = lowerText.range(of: keyword, options: .caseInsensitive, range: searchRange) else {
                    break
                }

                // 转换回原始文本的范围
                if let originalRange = rangeInOriginalText(text, lowerRange: range) {
                    allRanges.append(originalRange)
                }
